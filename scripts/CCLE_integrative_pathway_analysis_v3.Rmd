---
title: "CCLE_integrative_pathway_analysis_v3"
date: "6/8/2020"
...
x <- x %>% group_by(Target_Genes, phos) %>% top_n(1, wt=abs(trans_rppa_FC_log2))

y <- x[x$Target_Genes %in% rppa_phos_common_genes,]
y1 <- sapply(unique(y$Target_Genes), function(a) {
  b <- x %>% filter(Target_Genes == a)
  b$rppa_FC_log2[b$phos == 1] / b$rppa_FC_log2[b$phos == 0]
})
names(y1) <- unique(y$Target_Genes)
y2 <- y[seq(1,nrow(y),by = 2), c(1,4)]
y2$trans_rppa_FC_log2 <- y1

trans_RPPA_FC <- y2
trans_RPPA_FC <- rbind(trans_RPPA_FC, x[!x$Target_Genes %in% rppa_phos_common_genes,c(1,4,6)])
colnames(trans_RPPA_FC) <- c('Antibody', 'gene', 'trans_rppa_FC_log2')

# Put RNA and RPPA values together
combined_FC <- merge(RNA_FC, trans_RPPA_FC, by = 'gene', all = T)
...
x1 <- combined_FC[combined_FC$is_max == 0, c(1,2,4,3,6)]
colnames(x1)[4] <- 'max_FC_log2'
x2 <- combined_FC[combined_FC$is_max == 1, c(1,2,4,5,6)]
...
write.csv(max_FC, file = 'CCLE_integrative/transformed_data/063020/max_FC_070120.csv')
```
...
x %>% dplyr::select(gene, rna_FC_log2, trans_rppa_FC_log2) %>% reshape::melt(id='gene') %>% ggplot(aes(variable, value, color = variable)) + geom_boxplot()
...
write.table(rna_gene_FC[,2:3], file = 'CCLE_integrative/david_integrative_analysis/070120/rna_all_FC.txt', sep = '\t', row.names = F, quote = F)
```
...
write.table(all_gene_list, file = 'CCLE_integrative/david_integrative_analysis/070120/background_genes_070120.txt', sep = '\n', row.names = F, col.names = F)
write.table(up_gene_list, file = 'CCLE_integrative/david_integrative_analysis/070120/up_genes_070120.txt', sep = '\n', row.names = F, col.names = F)
write.table(down_gene_list, file = 'CCLE_integrative/david_integrative_analysis/070120/down_genes_070120.txt', sep = '\n', row.names = F, col.names = F)
```
...
write.table(FC_gene_list[,c(3,2)], file = 'CCLE_integrative/david_integrative_analysis/070120/transformed_FC.txt', sep = '\t', row.names = F, quote = F)
```
...
```{r}
up_DAVID_trans <- read.table('CCLE_integrative/david_integrative_analysis/070120/david/david_up_paths_int_070220.txt', sep = '\t', header = T, stringsAsFactors = F)
...
# KEGG 
up_DAVID_KEGG_trans <- up_DAVID_trans %>% filter(grepl("^KEGG_", Category) & Benjamini < .05 & Count < 500)
up_DAVID_KEGG_trans$term_filt <- sapply(up_DAVID_KEGG_trans$Term, function(x) strsplit(x, split = '\\:')[[1]][1])
up_DAVID_KEGG_trans$description <- sapply(up_DAVID_KEGG_trans$Term, function(x) strsplit(x, split = '\\:')[[1]][2])

up_DAVID_KEGG_trans %>% arrange(Count) %>% top_n(20, wt=Benjamini) %>% ggplot(aes(Count, reorder(description, Count), color = Benjamini, size = Count)) + geom_point() + xlab('Gene Count') + ylab('Pathways') + ggtitle('Top Down Transformed Integrative KEGG Pathways (DAVID)') + theme_bw()

# # REACT (no pathways)
# up_DAVID_REACT_trans <- up_DAVID_trans %>% filter(grepl("^REACTOME_", Category) & Benjamini < .05 & Count < 500)
# up_DAVID_REACT_trans$term_filt <- sapply(up_DAVID_REACT_trans$Term, function(x) strsplit(x, split = '\\:')[[1]][1])
# up_DAVID_REACT_trans$description <- sapply(up_DAVID_REACT_trans$Term, function(x) strsplit(x, split = '\\:')[[1]][2])
# 
# up_DAVID_REACT_trans %>% arrange(Count) %>% top_n(20, wt=Benjamini) %>% ggplot(aes(Count, reorder(description, Count), color = Benjamini, size = Count)) + geom_point() + xlab('Gene Count') + ylab('Pathways') + ggtitle('Top Down Transformed Integrative REACTOME Pathways (DAVID)') + theme_bw()

```

```{r}
down_DAVID_trans <- read.table('CCLE_integrative/david_integrative_analysis/070120/david/david_down_paths_int_070220.txt', sep = '\t', header = T, stringsAsFactors = F)
...
# KEGG 
down_DAVID_KEGG_trans <- down_DAVID_trans %>% filter(grepl("^KEGG_", Category) & Benjamini < .05 & Count < 500)
down_DAVID_KEGG_trans$term_filt <- sapply(down_DAVID_KEGG_trans$Term, function(x) strsplit(x, split = '\\:')[[1]][1])
down_DAVID_KEGG_trans$description <- sapply(down_DAVID_KEGG_trans$Term, function(x) strsplit(x, split = '\\:')[[1]][2])

down_DAVID_KEGG_trans %>% arrange(Count) %>% top_n(20, wt=Benjamini) %>% ggplot(aes(Count, reorder(description, Count), color = Benjamini, size = Count)) + geom_point() + xlab('Gene Count') + ylab('Pathways') + ggtitle('Top Down Transformed Integrative KEGG Pathways (DAVID)') + theme_bw()

# REACT (no pathways)
down_DAVID_REACT_trans <- down_DAVID_trans %>% filter(grepl("^REACTOME_", Category) & Benjamini < .05 & Count < 500)
down_DAVID_REACT_trans$term_filt <- sapply(down_DAVID_REACT_trans$Term, function(x) strsplit(x, split = '\\:')[[1]][1])
down_DAVID_REACT_trans$description <- sapply(down_DAVID_REACT_trans$Term, function(x) strsplit(x, split = '\\:')[[1]][2])

down_DAVID_REACT_trans %>% arrange(Count) %>% top_n(20, wt=Benjamini) %>% ggplot(aes(Count, reorder(description, Count), color = Benjamini, size = Count)) + geom_point() + xlab('Gene Count') + ylab('Pathways') + ggtitle('Top Down Transformed Integrative REACTOME Pathways (DAVID)') + theme_bw()
```
...
``` {r}
common_paths <- up_DAVID_KEGG_rna$term_filt[up_DAVID_KEGG_rna$term_filt %in% up_DAVID_KEGG_trans$term_filt]
trans_common_path <- up_DAVID_KEGG_rna[match(common_paths, up_DAVID_KEGG_rna$term_filt), c(1, 14, 15, 4, 12)]
trans_common_path <- trans_common_path %>% mutate(X. = X./100)
x <- up_DAVID_KEGG_trans[match(common_paths, up_DAVID_KEGG_trans$term_filt), c(4, 12)]
x <- x %>% mutate(X. = X./100)
trans_common_path <- cbind(trans_common_path, x)
colnames(trans_common_path) <- c('Category', 'Term', 'Description', 'rna_ratio', 'rna_benjamini', 'int_ratio', 'int_benjamini')
...
x %>% filter(Term %in% y$Term) %>% ggplot(aes(ratio_val, reorder(Description, -benjamini_val), fill = ratio_var)) + geom_bar(stat = 'identity', position = 'dodge') + ylab('Gene Ratio') + xlab('Pathways') + ggtitle('Integrative vs. RNA Top Up DAVID KEGG Pathways') + theme_bw()

x %>% filter(Term %in% y$Term) %>% ggplot(aes(ratio_var, reorder(Description, -benjamini_val), color = -log10(benjamini_val), size = ratio_val)) + geom_point() + ylab('Gene Ratio') + xlab('Pathways') + ggtitle('Transformed Integrative vs. RNA Top Up DAVID KEGG Pathways (FC)') + theme_bw()

## down
common_paths <- down_DAVID_KEGG_rna$term_filt[down_DAVID_KEGG_rna$term_filt %in% down_DAVID_KEGG_trans$term_filt]
trans_common_path <- down_DAVID_KEGG_rna[match(common_paths, down_DAVID_KEGG_rna$term_filt), c(1, 14, 15, 4, 12)]
trans_common_path <- trans_common_path %>% mutate(X. = X./100)
x <- down_DAVID_KEGG_trans[match(common_paths, down_DAVID_KEGG_trans$term_filt), c(4, 12)]
x <- x %>% mutate(X. = X./100)
trans_common_path <- cbind(trans_common_path, x)
colnames(trans_common_path) <- c('Category', 'Term', 'Description', 'rna_ratio', 'rna_benjamini', 'int_ratio', 'int_benjamini')
...
x %>% filter(Term %in% y$Term) %>% ggplot(aes(ratio_val, reorder(Description, -benjamini_val), fill = ratio_var)) + geom_bar(stat = 'identity', position = 'dodge') + ylab('Gene Ratio') + xlab('Pathways') + ggtitle('Integrative vs. RNA Top Down DAVID KEGG Pathways') + theme_bw()

x %>% filter(Term %in% y$Term) %>% ggplot(aes(ratio_var, reorder(Description, -benjamini_val), color = -log10(benjamini_val), size = ratio_val)) + geom_point() + ylab('Gene Ratio') + xlab('Pathways') + ggtitle('Transformed Integrative vs. RNA Top Down DAVID KEGG Pathways (FC)') + theme_bw()
```
...
```{r}
# DE for RPPA (limma)
# Get expression matrix (on log2 because assuming normal distribution/linear)
CCLE_RgPPA_breast_orig <- CCLE_RPPA_breast[-1,-1]
CCLE_RPPA_breast_orig <- as.data.frame(apply(CCLE_RPPA_breast_orig, 2, function(x) as.numeric(x)))
dimnames(CCLE_RPPA_breast_orig) <- list(CCLE_RPPA_breast[-1,1], CCLE_RPPA_breast[1,-1])

# limma analysis 
groups <- factor(line_metadata_filt$ERsign, levels = c('neg', 'pos'))
design <- model.matrix(~0+groups)
rppa_fit <- lmFit(t(CCLE_RPPA_breast_orig), design)
cont.matrix <- makeContrasts(negvspos = groupspos-groupsneg, levels = design)
rppa_fit2 <- contrasts.fit(rppa_fit, cont.matrix)
rppa_fit2 <- eBayes(rppa_fit2, trend = T, robust = T)
rppa_results <- decideTests(rppa_fit2)
summary(rppa_results)

# DE table
rppa_DE <- topTable(rppa_fit2, number = Inf, p.value = 1)
rppa_DE$gene <- CCLE_RPPA_genes$Target_Genes[match(rownames(rppa_DE), CCLE_RPPA_genes$Antibody_Name)]

# Significant DEs
rppa_DE_sig <- rppa_DE[rppa_DE$adj.P.Val < .05,]
unique(rppa_DE_sig$gene)
unique(rppa_DE_sig$gene[duplicated(rppa_DE_sig$gene)])

# Combining total and phosphorylated values
## Get duplicated genes (with total and phosphorylated values)
dup_genes <- unique(rppa_DE_sig$gene[duplicated(rppa_DE_sig$gene)])

## Get genes that are not duplicated
rppa_DE_sig_combine <- rppa_DE_sig %>% mutate(phos = ifelse(grepl("_p", rownames(rppa_DE_sig)), yes = 1, no = 0)) %>% filter(!gene %in% dup_genes) %>% dplyr::select(logFC, gene)

## Combine duplicated genes logFC
temp_frame <- data.frame()
for (current_gene in dup_genes) {
  temp <- rppa_DE_sig %>% rownames_to_column() %>% filter(gene == current_gene) %>% mutate(phos = ifelse(grepl("_p", rowname), yes = 1, no = 0))
  temp_frame <- rbind(temp_frame, data.frame(logFC = (sign(temp$logFC[1]) * temp$logFC[temp$phos == 1] / temp$logFC[temp$phos == 0]), gene = current_gene))
}

## Add to combined dataset and find direction of expression
rppa_DE_sig_combine <- rbind(rppa_DE_sig_combine, temp_frame)
rppa_DE_sig_combine <- rppa_DE_sig_combine %>% mutate(direction = ifelse(logFC > 0, yes = 'pos', no = 'neg'))

```

### Setup for Pathway Analysis
#### Integrative
```{r}
# all genes list
all_rna_genes <- unique(rna_gene_names$gene)
all_rppa_genes <- unique(CCLE_RPPA_genes$Target_Genes)
all_genes <- unique(c(all_rna_genes, all_rppa_genes))

# Get RNA DE genes
rna_DE_genes <- data.frame(ensembl = rownames(res_data_filt), gene = rna_gene_names$gene[match(rownames(res_data_filt), rna_gene_names$ensembl)], log2FC = res_data_filt$log2FoldChange, stringsAsFactors = F)
rna_DE_genes <- rna_DE_genes %>% mutate(direction = ifelse(res_data_filt$log2FoldChange > 0, yes = 'pos', no = 'neg')) %>% filter(abs(res_data_filt$log2FoldChange) > 2) %>% filter(!is.na(gene))

up_DE_genes <- unique(c(rppa_DE_sig_combine$gene[rppa_DE_sig_combine$direction == 'pos'], rna_DE_genes$gene[rna_DE_genes$direction == 'pos']))
down_DE_genes <- unique(c(rppa_DE_sig_combine$gene[rppa_DE_sig_combine$direction == 'neg'], rna_DE_genes$gene[rna_DE_genes$direction == 'neg']))

# Convert to Entrez IDs
entrez_gene_DE_up <- AnnotationDbi::select(org.Hs.eg.db, keys = up_DE_genes, columns = 'ENTREZID', keytype = 'SYMBOL')
entrez_gene_DE_up <- entrez_gene_DE_up %>% filter(!is.na(ENTREZID))

entrez_gene_DE_down <- AnnotationDbi::select(org.Hs.eg.db, keys = down_DE_genes, columns = 'ENTREZID', keytype = 'SYMBOL')
entrez_gene_DE_down <- entrez_gene_DE_down %>% filter(!is.na(ENTREZID))

entrez_gene_all <- AnnotationDbi::select(org.Hs.eg.db, keys = all_genes, columns = 'ENTREZID', keytype = 'SYMBOL')
entrez_gene_all <- entrez_gene_all %>% filter(!is.na(ENTREZID))

# Export for pathway analysis (DAVID)
write.table(as.numeric(entrez_gene_all$ENTREZID), file = 'CCLE_integrative/david_untransformed_analysis/background_genes_int_061520.txt', sep = '\n', row.names = F, col.names = F)
write.table(as.numeric(entrez_gene_DE_up$ENTREZID), file = 'CCLE_integrative/david_untransformed_analysis/up_genes_int_061520.txt', sep = '\n', row.names = F, col.names = F)
write.table(as.numeric(entrez_gene_DE_down$ENTREZID), file = 'CCLE_integrative/david_untransformed_analysis/down_genes_int_061520.txt', sep = '\n', row.names = F, col.names = F)

# Export FC
x <- rppa_DE_sig_combine %>% select(gene, logFC) %>% mutate(data_type = 'RPPA') %>% dplyr::rename(log2FC = logFC)
x <- rbind(x, rna_DE_genes %>% select(gene, log2FC) %>% mutate(data_type = 'RNA'))
x <- x %>% group_by(gene) %>% top_n(1, wt=abs(log2FC))

write.table(x[,1:2], file = 'CCLE_integrative/david_untransformed_analysis/untransformed_FC.txt', sep = '\t', row.names = F, quote = F)
...
```{r}
# Convert to Entrez IDs
entrez_gene_DE_up <- AnnotationDbi::select(org.Hs.eg.db, keys = rna_DE_genes$gene[rna_DE_genes$direction == 'pos'], columns = 'ENTREZID', keytype = 'SYMBOL')
entrez_gene_DE_up <- entrez_gene_DE_up %>% filter(!is.na(ENTREZID))

entrez_gene_DE_down <- AnnotationDbi::select(org.Hs.eg.db, keys = rna_DE_genes$gene[rna_DE_genes$direction == 'neg'], columns = 'ENTREZID', keytype = 'SYMBOL')
entrez_gene_DE_down <- entrez_gene_DE_down %>% filter(!is.na(ENTREZID))

entrez_gene_all <- AnnotationDbi::select(org.Hs.eg.db, keys = all_rna_genes, columns = 'ENTREZID', keytype = 'SYMBOL')
entrez_gene_all <- entrez_gene_all %>% filter(!is.na(ENTREZID))


# Export for pathway analysis (DAVID)
write.table(as.numeric(entrez_gene_all$ENTREZID), file = 'CCLE_integrative/david_untransformed_analysis/background_genes_rna_061620.txt', sep = '\n', row.names = F, col.names = F)
write.table(as.numeric(entrez_gene_DE_up$ENTREZID), file = 'CCLE_integrative/david_untransformed_analysis/up_genes_rna_061620.txt', sep = '\n', row.names = F, col.names = F)
write.table(as.numeric(entrez_gene_DE_down$ENTREZID), file = 'CCLE_integrative/david_untransformed_analysis/down_genes_rna_061620.txt', sep = '\n', row.names = F, col.names = F)

# Export FC
x <- rna_DE_genes %>% dplyr::select(gene, log2FC) %>% group_by(gene) %>% top_n(1, wt=abs(log2FC))
write.table(x, file = 'CCLE_integrative/david_untransformed_analysis/rna_untransformed_FC.txt', sep = '\t', row.names = F, quote = F)
...
#### Integrative
```{r}
up_DAVID_untrans <- read.table('CCLE_integrative/david_untransformed_analysis/david_up_int_pathways_061620.txt', sep = '\t', header = T, stringsAsFactors = F)

# GO
up_DAVID_GO_untrans <- up_DAVID_untrans %>% filter(grepl("^GOTERM_", Category) & Benjamini < .05 & Count < 500)
up_DAVID_GO_untrans$term_filt <- sapply(up_DAVID_GO_untrans$Term, function(x) strsplit(x, split = '\\~')[[1]][1])
up_DAVID_GO_untrans$description <- sapply(up_DAVID_GO_untrans$Term, function(x) strsplit(x, split = '\\~')[[1]][2])

up_DAVID_GO_untrans %>% arrange(Count) %>% top_n(20, wt=Benjamini) %>% ggplot(aes(Count, reorder(description, Count), color = -log10(Benjamini), size = Count)) + geom_point() + xlab('Gene Count') + ylab('Pathways') + ggtitle('Top Down Integrative GO Pathways (DAVID)') + theme_bw()

# KEGG
up_DAVID_KEGG_untrans <- up_DAVID_untrans %>% filter(grepl("^KEGG_", Category) & Benjamini < .05 & Count < 500)
up_DAVID_KEGG_untrans$term_filt <- sapply(up_DAVID_KEGG_untrans$Term, function(x) strsplit(x, split = '\\:')[[1]][1])
up_DAVID_KEGG_untrans$description <- sapply(up_DAVID_KEGG_untrans$Term, function(x) strsplit(x, split = '\\:')[[1]][2])

up_DAVID_KEGG_untrans %>% arrange(Count) %>% top_n(20, wt=Benjamini) %>% ggplot(aes(Count, reorder(description, Count), color = -log10(Benjamini), size = Count)) + geom_point() + xlab('Gene Count') + ylab('Pathways') + ggtitle('Top Down 20 Integrative KEGG Pathways (DAVID)') + theme_bw()

# Reactome (no pathways)
# up_DAVID_REACT_untrans <- up_DAVID_untrans %>% filter(grepl("^REACTOME_", Category) & Benjamini < .05 & Count < 500)
# up_DAVID_REACT_untrans$term_filt <- sapply(up_DAVID_REACT_untrans$Term, function(x) strsplit(x, split = '\\:')[[1]][1])
# up_DAVID_REACT_untrans$description <- sapply(up_DAVID_REACT_untrans$Term, function(x) strsplit(x, split = '\\:')[[1]][2])
# 
# up_DAVID_REACT_untrans %>% arrange(Count) %>% top_n(20, wt=Benjamini) %>% ggplot(aes(Count, reorder(description, Count), color = Benjamini, size = Count)) + geom_point() + xlab('Gene Count') + ylab('Pathways') + ggtitle('Top Down 20 Integrative REACTOME Pathways (DAVID)') + theme_bw()

```

```{r}
down_DAVID_untrans <- read.table('CCLE_integrative/david_untransformed_analysis/david_down_int_pathways_061620.txt', sep = '\t', header = T, stringsAsFactors = F)

# GO
down_DAVID_GO_untrans <- down_DAVID_untrans %>% filter(grepl("^GOTERM_", Category) & Benjamini < .05 & Count < 500)
down_DAVID_GO_untrans$term_filt <- sapply(down_DAVID_GO_untrans$Term, function(x) strsplit(x, split = '\\~')[[1]][1])
down_DAVID_GO_untrans$description <- sapply(down_DAVID_GO_untrans$Term, function(x) strsplit(x, split = '\\~')[[1]][2])

down_DAVID_GO_untrans %>% arrange(Count) %>% top_n(20, wt=Benjamini) %>% ggplot(aes(Count, reorder(description, Count), color = Benjamini, size = Count)) + geom_point() + xlab('Gene Count') + ylab('Pathways') + ggtitle('Top Down Integrative GO Pathways (DAVID)') + theme_bw()

# KEGG
down_DAVID_KEGG_untrans <- down_DAVID_untrans %>% filter(grepl("^KEGG_", Category) & Benjamini < .05 & Count < 500)
down_DAVID_KEGG_untrans$term_filt <- sapply(down_DAVID_KEGG_untrans$Term, function(x) strsplit(x, split = '\\:')[[1]][1])
down_DAVID_KEGG_untrans$description <- sapply(down_DAVID_KEGG_untrans$Term, function(x) strsplit(x, split = '\\:')[[1]][2])

down_DAVID_KEGG_untrans %>% arrange(Count) %>% top_n(20, wt=Benjamini) %>% ggplot(aes(Count, reorder(description, Count), color = Benjamini, size = Count)) + geom_point() + xlab('Gene Count') + ylab('Pathways') + ggtitle('Top Down 20 Integrative KEGG Pathways (DAVID)') + theme_bw()

# Reactome 
down_DAVID_REACT_untrans <- down_DAVID_untrans %>% filter(grepl("^REACTOME_", Category) & Benjamini < .05 & Count < 500)
down_DAVID_REACT_untrans$term_filt <- sapply(down_DAVID_REACT_untrans$Term, function(x) strsplit(x, split = '\\:')[[1]][1])
down_DAVID_REACT_untrans$description <- sapply(down_DAVID_REACT_untrans$Term, function(x) strsplit(x, split = '\\:')[[1]][2])

down_DAVID_REACT_untrans %>% arrange(Count) %>% top_n(20, wt=Benjamini) %>% ggplot(aes(Count, reorder(description, Count), color = Benjamini, size = Count)) + geom_point() + xlab('Gene Count') + ylab('Pathways') + ggtitle('Top Down 20 Integrative REACTOME Pathways (DAVID)') + theme_bw()

```

#### RNA only
```{r}
up_DAVID_rna <- read.table('CCLE_integrative/david_untransformed_analysis/david_up_rna_pathways_061620.txt', sep = '\t', header = T, stringsAsFactors = F)
...
# KEGG
up_DAVID_KEGG_rna <- up_DAVID_rna %>% filter(grepl("^KEGG_", Category) & Benjamini < .05 & Count < 500)
up_DAVID_KEGG_rna$term_filt <- sapply(up_DAVID_KEGG_rna$Term, function(x) strsplit(x, split = '\\:')[[1]][1])
up_DAVID_KEGG_rna$description <- sapply(up_DAVID_KEGG_rna$Term, function(x) strsplit(x, split = '\\:')[[1]][2])

up_DAVID_KEGG_rna %>% arrange(Count) %>% top_n(20, wt=Benjamini) %>% ggplot(aes(Count, reorder(description, Count), color = Benjamini, size = Count)) + geom_point() + xlab('Gene Count') + ylab('Pathways') + ggtitle('Top Down RNA KEGG Pathways (DAVID)') + theme_bw()

# Reactome (no pathways)
# up_DAVID_REACT_rna <- up_DAVID_rna %>% filter(grepl("^REACTOME_", Category) & Benjamini < .05 & Count < 500)
# up_DAVID_REACT_rna$term_filt <- sapply(up_DAVID_REACT_rna$Term, function(x) strsplit(x, split = '\\:')[[1]][1])
# up_DAVID_REACT_rna$description <- sapply(up_DAVID_REACT_rna$Term, function(x) strsplit(x, split = '\\:')[[1]][2])
# 
# up_DAVID_REACT_rna %>% arrange(Count) %>% top_n(20, wt=Benjamini) %>% ggplot(aes(Count, reorder(description, Count), color = Benjamini, size = Count)) + geom_point() + xlab('Gene Count') + ylab('Pathways') + ggtitle('Top Down RNA REACTOME Pathways (DAVID)') + theme_bw()
```
 
```{r}
down_DAVID_rna <- read.table('CCLE_integrative/david_untransformed_analysis/david_down_rna_pathways_061620.txt', sep = '\t', header = T, stringsAsFactors = F)
...
# KEGG
down_DAVID_KEGG_rna <- down_DAVID_rna %>% filter(grepl("^KEGG_", Category) & Benjamini < .05 & Count < 500)
down_DAVID_KEGG_rna$term_filt <- sapply(down_DAVID_KEGG_rna$Term, function(x) strsplit(x, split = '\\:')[[1]][1])
down_DAVID_KEGG_rna$description <- sapply(down_DAVID_KEGG_rna$Term, function(x) strsplit(x, split = '\\:')[[1]][2])

down_DAVID_KEGG_rna %>% arrange(Count) %>% top_n(20, wt=Benjamini) %>% ggplot(aes(Count, reorder(description, Count), color = Benjamini, size = Count)) + geom_point() + xlab('Gene Count') + ylab('Pathways') + ggtitle('Top Down RNA KEGG Pathways (DAVID)') + theme_bw()

# Reactome (no pathways)
down_DAVID_REACT_rna <- down_DAVID_rna %>% filter(grepl("^REACTOME_", Category) & Benjamini < .05 & Count < 500)
down_DAVID_REACT_rna$term_filt <- sapply(down_DAVID_REACT_rna$Term, function(x) strsplit(x, split = '\\:')[[1]][1])
down_DAVID_REACT_rna$description <- sapply(down_DAVID_REACT_rna$Term, function(x) strsplit(x, split = '\\:')[[1]][2])

down_DAVID_REACT_rna %>% arrange(Count) %>% top_n(20, wt=Benjamini) %>% ggplot(aes(Count, reorder(description, Count), color = Benjamini, size = Count)) + geom_point() + xlab('Gene Count') + ylab('Pathways') + ggtitle('Top Down RNA REACTOME Pathways (DAVID)') + theme_bw()
```

#### Common Integrative vs. RNA
```{r}
# GO
## up
common_paths <- up_DAVID_GO_rna$term_filt[up_DAVID_GO_rna$term_filt %in% up_DAVID_GO_untrans$term_filt]
untrans_common_path <- up_DAVID_GO_rna[match(common_paths, up_DAVID_GO_rna$term_filt), c(1, 14, 15, 3, 12)]
untrans_common_path <- cbind(untrans_common_path, up_DAVID_GO_untrans[match(common_paths, up_DAVID_GO_untrans$term_filt), c(3, 12)])
colnames(untrans_common_path) <- c('Category', 'Term', 'Description', 'rna_count', 'rna_benjamini', 'int_count', 'int_benjamini')

untrans_common_path %>% top_n(20, wt = rna_benjamini) %>% dplyr::select(Description, rna_count, int_count) %>% reshape2::melt('Description') %>% ggplot(aes(value, reorder(Description, value), fill = variable)) + geom_bar(stat = 'identity', position = 'dodge') + ylab('Gene Count') + xlab('Pathways') + ggtitle('Integrative vs. RNA Top Down GO Pathways') + theme_bw()

## down
common_paths <- down_DAVID_GO_rna$term_filt[down_DAVID_GO_rna$term_filt %in% down_DAVID_GO_untrans$term_filt]
untrans_common_path <- down_DAVID_GO_rna[match(common_paths, down_DAVID_GO_rna$term_filt), c(1, 14, 15, 3, 12)]
untrans_common_path <- cbind(untrans_common_path, down_DAVID_GO_untrans[match(common_paths, down_DAVID_GO_untrans$term_filt), c(3, 12)])
colnames(untrans_common_path) <- c('Category', 'Term', 'Description', 'rna_count', 'rna_benjamini', 'int_count', 'int_benjamini')

untrans_common_path %>% top_n(20, wt = rna_benjamini) %>% dplyr::select(Description, rna_count, int_count) %>% reshape2::melt('Description') %>% ggplot(aes(value, reorder(Description, value), fill = variable)) + geom_bar(stat = 'identity', position = 'dodge') + ylab('Gene Count') + xlab('Pathways') + ggtitle('Integrative vs. RNA Top Down GO Pathways') + theme_bw()

untrans_common_path %>% top_n(20, wt = rna_benjamini) %>% dplyr::select(Description, rna_count, int_count) %>% reshape2::melt('Description') %>% ggplot(aes(variable, reorder(Description, value), color = value, size = value)) + geom_point() + ylab('Gene Count') + xlab('Pathways') + ggtitle('Integrative vs. RNA Top Down GO Pathways') + theme_bw()
```

```{r}
# KEGG
## up
common_paths <- up_DAVID_KEGG_rna$term_filt[up_DAVID_KEGG_rna$term_filt %in% up_DAVID_KEGG_untrans$term_filt]
untrans_common_path <- up_DAVID_KEGG_rna[match(common_paths, up_DAVID_KEGG_rna$term_filt), c(1, 14, 15, 3, 12)]
untrans_common_path <- cbind(untrans_common_path, up_DAVID_KEGG_untrans[match(common_paths, up_DAVID_KEGG_untrans$term_filt), c(3, 12)])
colnames(untrans_common_path) <- c('Category', 'Term', 'Description', 'rna_count', 'rna_benjamini', 'int_count', 'int_benjamini')

untrans_common_path %>% top_n(20, wt = rna_benjamini) %>% dplyr::select(Description, rna_count, int_count) %>% reshape2::melt('Description') %>% ggplot(aes(value, reorder(Description, value), fill = variable)) + geom_bar(stat = 'identity', position = 'dodge') + ylab('Gene Count') + xlab('Pathways') + ggtitle('Integrative vs. RNA Top Down KEGG Pathways') + theme_bw()

untrans_common_path %>% top_n(20, wt = rna_benjamini) %>% dplyr::select(Description, rna_count, int_count) %>% reshape2::melt('Description') %>% ggplot(aes(variable, reorder(Description, value), color = value, size = value)) + geom_point() + ylab('Gene Count') + xlab('Pathways') + ggtitle('Integrative vs. RNA Top Down KEGG Pathways') + theme_bw()

## down
common_paths <- down_DAVID_KEGG_rna$term_filt[down_DAVID_KEGG_rna$term_filt %in% down_DAVID_KEGG_untrans$term_filt]
untrans_common_path <- down_DAVID_KEGG_rna[match(common_paths, down_DAVID_KEGG_rna$term_filt), c(1, 14, 15, 3, 12)]
untrans_common_path <- cbind(untrans_common_path, down_DAVID_KEGG_untrans[match(common_paths, down_DAVID_KEGG_untrans$term_filt), c(3, 12)])
colnames(untrans_common_path) <- c('Category', 'Term', 'Description', 'rna_count', 'rna_benjamini', 'int_count', 'int_benjamini')
...
# REACT
## down
common_paths <- down_DAVID_REACT_rna$term_filt[down_DAVID_REACT_rna$term_filt %in% down_DAVID_REACT_untrans$term_filt]
untrans_common_path <- down_DAVID_REACT_rna[match(common_paths, down_DAVID_REACT_rna$term_filt), c(1, 14, 15, 3, 12)]
untrans_common_path <- cbind(untrans_common_path, down_DAVID_REACT_untrans[match(common_paths, down_DAVID_REACT_untrans$term_filt), c(3, 12)])
colnames(untrans_common_path) <- c('Category', 'Term', 'Description', 'rna_count', 'rna_benjamini', 'int_count', 'int_benjamini')

untrans_common_path %>% top_n(20, wt = rna_benjamini) %>% dplyr::select(Description, rna_count, int_count) %>% reshape2::melt('Description') %>% ggplot(aes(value, reorder(Description, value), fill = variable)) + geom_bar(stat = 'identity', position = 'dodge') + ylab('Gene Count') + xlab('Pathways') + ggtitle('Integrative vs. RNA Top down REACTOME Pathways') + theme_bw()

untrans_common_path %>% top_n(20, wt = rna_benjamini) %>% dplyr::select(Description, rna_count, int_count) %>% reshape2::melt('Description') %>% ggplot(aes(variable, reorder(Description, value), color = value, size = value)) + geom_point() + ylab('Gene Count') + xlab('Pathways') + ggtitle('Integrative vs. RNA Top down REACTOME Pathways') + theme_bw()

```

#### Different Integrative vs. RNA
```{r}
diff_paths_int <- setdiff(up_DAVID_untrans$Term, up_DAVID_rna$Term)
diff_paths_rna <- setdiff(up_DAVID_rna$Term, up_DAVID_untrans$Term)
untrans_diff_path <- up_DAVID_untrans %>% filter(Term %in% diff_paths_int) %>% dplyr::select(Term, Count, Benjamini) %>% mutate(data_type = 'int')
untrans_diff_path <- rbind(untrans_diff_path, up_DAVID_rna %>% filter(Term %in% diff_paths_rna) %>% dplyr::select(Term, Count, Benjamini) %>% mutate(data_type = 'rna'))

# note: not much difference for DAVID between untransformed integrative and rna datasets
```

### MetaCore Analysis
#### Integrative
```{r}
# Overenrichment (no FC)
## up
metacore_up <- read.csv('CCLE_integrative/david_untransformed_analysis/metacore_ccle_up_untrans_pathways_all_061920.csv', skip = 2, stringsAsFactors = F, header = T)
metacore_up <- metacore_up[-nrow(metacore_up),-c(1,ncol(metacore_up))]
colnames(metacore_up) <- c('Maps', 'Total', 'min_pvalue', 'min_FDR', 'int_pvalue', 'int_FDR', 'int_count', 'int_genes', 'rna_pvalue', 'rna_FDR', 'rna_count', 'rna_genes')
metacore_up <- metacore_up %>% mutate(int_ratio = int_count / Total, rna_ratio = rna_count / Total)

x1 <- reshape::melt(metacore_up[,c('Maps', 'int_ratio', 'rna_ratio')], id = 'Maps')
x2 <- reshape::melt(metacore_up[,c('Maps', 'int_FDR', 'rna_FDR')], id = 'Maps')
x <- cbind(x1, x2[,c(2:3)])
colnames(x) <- c('Maps', 'ratio_var', 'ratio_val', 'FDR_var', 'FDR_val')

y <- metacore_up %>% top_n(-20, wt=min_FDR)

x %>% filter(Maps %in% y$Maps) %>% ggplot(aes(ratio_val, reorder(Maps, -FDR_val), fill = ratio_var)) + geom_bar(stat = 'identity', position = 'dodge') + ylab('Gene Ratio') + xlab('Pathways') + ggtitle('Integrative vs. RNA Top Down Metacore Pathways') + theme_bw()

x %>% filter(Maps %in% y$Maps) %>% ggplot(aes(ratio_var, reorder(Maps, -FDR_val), color = -log10(FDR_val), size = ratio_val)) + geom_point() + ylab('Gene Ratio') + xlab('Pathways') + ggtitle('Integrative vs. RNA Top Down Metacore Pathways') + theme_bw()

## down
metacore_down <- read.csv('CCLE_integrative/david_untransformed_analysis/metacore_ccle_down_untrans_pathways_061820.csv', skip = 2, stringsAsFactors = F, header = T)
metacore_down <- metacore_down[-nrow(metacore_down),-c(1,ncol(metacore_down))]
colnames(metacore_down) <- c('Maps', 'Total', 'min_pvalue', 'min_FDR', 'int_pvalue', 'int_FDR', 'int_count', 'int_genes', 'rna_pvalue', 'rna_FDR', 'rna_count', 'rna_genes')
metacore_down <- metacore_down %>% mutate(int_ratio = int_count / Total, rna_ratio = rna_count / Total)

x1 <- reshape::melt(metacore_down[,c('Maps', 'int_ratio', 'rna_ratio')], id = 'Maps')
x2 <- reshape::melt(metacore_down[,c('Maps', 'int_FDR', 'rna_FDR')], id = 'Maps')
x <- cbind(x1, x2[,c(2:3)])
colnames(x) <- c('Maps', 'ratio_var', 'ratio_val', 'FDR_var', 'FDR_val')

y <- metacore_down %>% top_n(-20, wt=min_FDR)

x %>% filter(Maps %in% y$Maps) %>% ggplot(aes(ratio_val, reorder(Maps, -FDR_val), fill = ratio_var)) + geom_bar(stat = 'identity', position = 'dodge') + ylab('Gene Ratio') + xlab('Pathways') + ggtitle('Integrative vs. RNA Top Down Metacore Pathways') + theme_bw()

x %>% filter(Maps %in% y$Maps) %>% ggplot(aes(ratio_var, reorder(Maps, ratio_val), color = -log10(FDR_val), size = ratio_val)) + geom_point() + ylab('Gene Ratio') + xlab('Pathways') + ggtitle('Integrative vs. RNA Top Down Metacore Pathways') + theme_bw()

```

```{r}
# FC
metacore_FC <- read.csv('CCLE_integrative/david_untransformed_analysis/metacore_ccle_untransformed_FC_pathways_061920.csv', skip = 2, stringsAsFactors = F, header = T)
metacore_FC <- metacore_FC[,-1]
colnames(metacore_FC) <- c('Maps', 'Total', 'min_pvalue', 'min_FDR', 'int_pvalue', 'int_FDR', 'int_count', 'int_genes', 'rna_pvalue', 'rna_FDR', 'rna_count', 'rna_genes')
metacore_FC <- metacore_FC %>% mutate(int_ratio = int_count / Total, rna_ratio = rna_count / Total)

x1 <- reshape::melt(metacore_FC[,c('Maps', 'int_ratio', 'rna_ratio')], id = 'Maps')
x2 <- reshape::melt(metacore_FC[,c('Maps', 'int_FDR', 'rna_FDR')], id = 'Maps')
x <- cbind(x1, x2[,c(2:3)])
colnames(x) <- c('Maps', 'ratio_var', 'ratio_val', 'FDR_var', 'FDR_val')

y <- metacore_FC %>% top_n(-20, wt=min_FDR)

x %>% filter(Maps %in% y$Maps) %>% ggplot(aes(ratio_val, reorder(Maps, -FDR_val), fill = ratio_var)) + geom_bar(stat = 'identity', position = 'dodge') + ylab('Gene Ratio') + xlab('Pathways') + ggtitle('Integrative vs. RNA Top Metacore Pathways (FC)') + theme_bw()

x %>% filter(Maps %in% y$Maps) %>% ggplot(aes(ratio_var, reorder(Maps, -FDR_val), color = -log10(FDR_val), size = ratio_val)) + geom_point() + ylab('Gene Ratio') + xlab('Pathways') + ggtitle('Integrative vs. RNA Top Down Metacore Pathways (FC)') + theme_bw()
