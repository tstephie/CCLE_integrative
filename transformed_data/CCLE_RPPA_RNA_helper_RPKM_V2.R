...
## RNASeq data input
CCLE_RNA3 <- read.csv("CCLE_RNAseq_genes_rpkm_20180929.csv", header = FALSE)

# Filter the RNASeq with only the genes that appear in RPPA
...
CCLE_RNA4 <- CCLE_RNA3 %>% filter(grepl(paste(keys, collapse="|"), V2))
...
## Find cell lines that dont match 
...
# Filter the RNA further with the cell line diff
...
ggplot(data = final)+
  ...
  geom_abline(slope = pos_m, intercept = pos_b, color = "red", size = 1) + 
  ...
  geom_abline(slope = neg_m, intercept = neg_b, color = "darkred", size = 1) + 
  annotate(geom = 'text', size = 5, x=1.7, y=10, label = 'Positive: y = 0.71x + 4.34', color = 'red', fontface = 2) +
  annotate(geom = 'text', size = 5, x=1.7, y=9.5, label = 'Negative: y = 1.49x + 4.25', color = 'darkred', fontface = 2) +
  xlab('log2(RPPA)') + ylab('log2(RNA+1)') + theme(axis.title.x = element_text(size = 15, face = 'bold'), axis.title.y = element_text(size = 15, face = 'bold'))

final_transform <- final[,c(1:5,8,9)]
write.csv(final_transform, "final_RPKM_transform_V2.csv")
...
phos_final <- read.csv("CCLE_integrative/transformed_data/RPKM_phos_final_V2.csv")
nonphos_final <- read.csv("CCLE_integrative/transformed_data/RPKM_nonphos_final_V2.csv")
#MAKE FCS, RECOMBO
...
#write.csv(all_final, "final_RPKM_RNA2RPPA_CCLE_analysis_V2.csv")
...
CCLE_RNA_only <- as.data.frame(t(CCLE_RNA3),stringsAsFactors = FALSE)
...
RNA_onlyfinal_RPKM <- left_join(RNA_onlypos, RNA_onlyneg)

RNA_onlyfinal_RPKM$posRNAaverage <- log2(RNA_onlyfinal_RPKM$posRNAaverage + 1)
RNA_onlyfinal_RPKM$negRNAaverage <- log2(RNA_onlyfinal_RPKM$negRNAaverage + 1)


RNA_onlyfinal_RPKM$RPPA_transform_pos <- (RNA_onlyfinal_RPKM$posRNAaverage - pos_b) / pos_m 
RNA_onlyfinal_RPKM$RPPA_transform_neg <- (RNA_onlyfinal_RPKM$negRNAaverage - neg_b) / neg_m

RNA_onlyfinal_RPKM$log2RPPAFC <- RNA_onlyfinal_RPKM$RPPA_transform_pos - RNA_onlyfinal_RPKM$RPPA_transform_neg
RNA_onlyfinal_RPKM$log2RNAFC <- RNA_onlyfinal_RPKM$posRNAaverage - RNA_onlyfinal_RPKM$negRNAaverage
RNA_onlyfinal_RPKM$RPPAFC <- 2^(RNA_onlyfinal_RPKM$log2RPPAFC)
RNA_onlyfinal_RPKM$RNAFC <- 2^(RNA_onlyfinal_RPKM$log2RNAFC)

RNA_onlyfinal_RPKM <- RNA_onlyfinal_RPKM[, c(1:6,9,10,7,8)]

#write.csv(RNA_onlyfinal_RPKM, "RPKM_RNA_only_transform_V2.csv")

ggplot(data = RNA_onlyfinal_RPKM)+
...
ggplot()+
  ...
  geom_point(data = RNA_onlyfinal_RPKM,aes(x = RPPA_transform_pos, y = posRNAaverage), color = "pink") +
  geom_point(data = RNA_onlyfinal_RPKM,aes(x = RPPA_transform_neg, y = negRNAaverage), color = "grey80")
